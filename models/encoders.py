...
class DualEncoder(nn.Module):
    ...
    def forward(self, inputs, num_hp=25):
        ...
        features=self.relu(features)
        instancecode=self.fc3(features)
        basecode=self.basecode.repeat(num_hp*bs,1)
        hpcode=self.hpcode.unsqueeze(1).repeat(1,bs,1)
        instancecode=instancecode.unsqueeze(0).repeat(num_hp,1,1)
        code=torch.cat([basecode.reshape(num_hp,bs,-1),hpcode,instancecode],dim=2)
