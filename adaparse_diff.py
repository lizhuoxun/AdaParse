...
ground_truth_loss_all=torch.from_numpy(np.load(opt.ground_truth_dir+ "DM_loss_3dim.npy"))
ground_truth_loss_9_all=torch.from_numpy(np.load(opt.ground_truth_dir+ "DM_loss.npy"))
...
os.makedirs('%s/result_diff/adapt/%s' % (save_dir, sig), exist_ok=True)
os.makedirs('%s/result_diff/adapt/%s/model' % (save_dir, sig), exist_ok=True)
...
weightn2 = torch.tensor([123/10, 123/81, 123/8,123/24])
weightn3 = torch.tensor([123/15, 123/81, 123/24,123/3])
weightn4 = torch.tensor([123/80, 123/43])
weightn5 = torch.tensor([123/39, 123/84])
weightn6 = torch.tensor([123/67, 123/56])

weight3L1 = torch.tensor([123/32, 123/91])
weight3L2 = torch.tensor([123/82, 123/41])
weight3L3 = torch.tensor([123/71, 123/52])

weight9L1 = torch.tensor([123/77, 123/46])
weight9L2 = torch.tensor([123/102, 123/21])
weight9L3 = torch.tensor([123/102, 123/21])
weight9L4 = torch.tensor([123/117, 123/6])
weight9L5 = torch.tensor([123/78, 123/45])
weight9L6 = torch.tensor([123/109, 123/14])
weight9L7 = torch.tensor([123/109, 123/14])
weight9L8 = torch.tensor([123/117, 123/6])
weight9L9 = torch.tensor([123/112, 123/11])
weight9L10 = torch.tensor([123/71, 123/52])
...
def train(batch, labels,g_t_net,g_t_loss, g_t_loss_9):
    ...
    zero_1=torch.zeros(hpfingerprints_gray.shape, dtype=torch.float32).to(device)

    loss1=0.1*l1(hpfingerprints_gray,zero_1).to(device)
    loss2=0.05*l1(low_freq_part,zero).to(device)
...
for epoch in range(epochs):
    ...
    torch.save(state, '%s/result_diff/adapt/%s/model/123_model_%d.pickle' % (save_dir, sig, epoch))
